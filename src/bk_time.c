...
BK_ClockFrame bk_clock_advance(BK_Clock *clock, u64 now_ns) {
  ...
  u64 ticks = uncapped_ticks < (u64)clock->max_ticks_per_frame ? uncapped_ticks
...
f64 bk_clock_sim_time(const BK_Clock *clock) {
  return (f64)clock->tick * bk_clock_fixed_dt(clock);
}
