...
/// Discards the table and rebuilds it from the record array, growing to keep the load
/// factor under 3/4. Returns false and logs on allocation failure, leaving the old table
/// intact so the caller can bail without corrupting state.
...
static void s_record_remove(BK_Atlas *atlas, i32 index) {
  ...
  atlas->record_count--;
  // Cannot fail meaningfully: the table only ever shrinks in load here, so the rebuild
  // reuses a size it already allocated once. A failure still leaves a consistent (empty)
  // table, and the next add rebuilds again.
  (void)s_map_rebuild(atlas);
}
```
...
Note that this keeps the buffer's allocation (`atlas->pushed` still owns it) and only resets the count — the entries are consumed in place before any further push can occur, because nothing between here and the end of this function calls back into `bk__atlas_push`. Say that in a comment; it is the one thing that makes reusing the buffer safe.
...
6. `create_texture(image, size, size)`. Free the image and the scratch buffer. If it returns 0: log, return -1 — and **do not destroy any lonely texture**, so every image is still drawable from where it was.
